...
shared_ptr<Literal> OperatorMinus::executeSpecificOperator() {
    try {
        ...
        if (comp_a != nullptr) {
            ...
            if (exp_b != nullptr) {
                /* Difference between Complex and Expression is in fact difference between two Expression :
                 *      We have to create a temporary Expression with the Complex::toString() function
                 *      and then apply difference between the two Expression */
