...
TEST_F(Test_Operator_Inferior, Real_Inf_Real_Gives_True) {
    /* integer < integer = 1 */
    st.push(shared_ptr<ComplexLiteral>(new ComplexLiteral(shared_ptr<NumericLiteral>(new NumericLiteral(2.1)))));
    st.push(shared_ptr<ComplexLiteral>(new ComplexLiteral(shared_ptr<NumericLiteral>(new NumericLiteral(3.1)))));
    op_inf.execute(&st);
    EXPECT_EQ(1, st.size());
    EXPECT_EQ("1", st.top()->toString());
}
...
TEST_F(Test_Operator_Inferior, Rational_Inf_Rational_Gives_True) {
    ...
    st.push(shared_ptr<ComplexLiteral>(new ComplexLiteral(shared_ptr<NumericLiteral>(new NumericLiteral(7,2)))));
    op_inf.execute(&st);
    EXPECT_EQ(1, st.size());
    EXPECT_EQ("1", st.top()->toString());
}
...
TEST_F(Test_Operator_Inferior, Integer_Sup_Real_Gives_False) {
    /* integer > integer = 0 */
    st.push(shared_ptr<ComplexLiteral>(new ComplexLiteral(shared_ptr<NumericLiteral>(new NumericLiteral(3.0)))));
    st.push(shared_ptr<ComplexLiteral>(new ComplexLiteral(shared_ptr<NumericLiteral>(new NumericLiteral(2.2)))));
    op_inf.execute(&st);
    EXPECT_EQ(1, st.size());
    EXPECT_EQ("0", st.top()->toString());
}
